...
class Giohang extends CI_Controller {
    ...
    public function index(){
        $this->data['title']='SShopDor- Giỏ hàng của bạn';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionKhachHang')){
            ...
            $this->email->from('sale.smart.store.2019@gmail.com', 'ShopDor');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('Hệ thống ShopDor');
        ...
        $this->data['title']='ShopDor.vn - Kết quả đơn hàng';
